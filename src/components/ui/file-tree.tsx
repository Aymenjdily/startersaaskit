/**
 * File tree — vendored from MagicUI (https://magicui.design/docs/components/file-tree).
 *
 * Copied rather than imported: MagicUI ships shadcn-style, as source you own.
 * It is third-party code, so it is left close to as published — the formatting
 * and idioms below are theirs, not this repo's, and that is deliberate so the
 * next version can be diffed against it.
 *
 * One deliberate deviation: its three `bg-muted` / `hover:bg-slate-300` colours
 * are shadcn *light-theme* tokens — `--muted` resolves to `oklch(0.967 …)` and
 * the indicator to `oklch(0.92 …)`, both near-white. On this console's dark
 * panels that is a glaring white block behind the selected file and a hard
 * white rule down the nesting guides. They are white-alpha here instead, which
 * reads as a tint at any panel lightness.
 *
 * Requires `@radix-ui/react-accordion` and `@radix-ui/react-scroll-area`, both
 * added for this.
 */
"use client";
...
>(({ className, ...props }, ref) => {
	...
	return (
		<div
			...
			ref={ref}
			className={cn(
				"absolute left-1.5 h-full w-px rounded-md bg-white/10 py-3 duration-300 ease-in-out hover:bg-white/25 rtl:right-1.5",
				className,
			)}
			{...props}
		/>
			...
			>
				<AccordionPrimitive.Trigger
					className={cn(
						...
						className,
						{
							"rounded-md bg-white/10": isSelected && isSelectable,
		...
		return (
			<button
				...
				className={cn(
					...
					{
						"bg-white/10": isSelected && isSelectable,
					},
